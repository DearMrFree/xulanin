...
import { ArrowRight, MapPin, MessageCircle, Truck, Star, Clock, Sparkles } from "lucide-react";
  ...
  return (
    <section id="home" className="min-h-screen relative overflow-hidden bg-hero-gradient">
      ...
      <div className="container mx-auto px-6 pt-32 pb-10 relative">
        {/* Trust badges */}
        <div className="flex flex-wrap items-center gap-3 mb-10 animate-fade-in-up">
          ...
          {/* Freshness / urgency badge */}
          <div className="flex items-center gap-2 text-sm bg-gradient-to-r from-amber-500 to-orange-500 text-white px-4 py-2 rounded-full shadow-md font-semibold animate-pulse-glow">
            <Sparkles size={14} />
            <span>Fresh Today · Orders close {XUBIE_DATA.company.orderCutoff}</span>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="lg:col-span-5 space-y-8">
            ...
            <p className="text-[var(--muted-foreground)] text-lg leading-relaxed max-w-lg animate-fade-in-up delay-300">
              ...
              desserts from the booth to your door. Order in minutes.
            </p>

            <div className="flex flex-col sm:flex-row gap-4 animate-fade-in-up delay-400">
              <Link
                href="/menu"
                className="inline-flex items-center justify-center gap-3 px-8 py-4 bg-[var(--primary)] text-[var(--primary-foreground)] rounded-full font-semibold shadow-primary hover:opacity-90 active:scale-[0.97] transition-all duration-200 text-base"
              >
                Order Now
                <ArrowRight size={18} />
              </Link>
              <a
                ...
                className="inline-flex items-center justify-center gap-3 px-8 py-4 bg-emerald-500 text-white rounded-full font-semibold hover:bg-emerald-600 active:scale-[0.97] transition-all duration-200 shadow-lg text-base"
              >
            ...
            {/* Pickup time badge */}
            <div className="flex items-center gap-3 animate-fade-in-up delay-450 bg-white/60 backdrop-blur-sm border border-[var(--border)] rounded-2xl px-4 py-3 max-w-xs">
              <Clock size={16} className="text-[var(--primary)] shrink-0" />
              <div>
                <p className="text-xs font-semibold text-[var(--foreground)]">Pickup ready in {XUBIE_DATA.company.pickupTime}</p>
                <p className="text-[11px] text-[var(--muted-foreground)]">2095 Fruitdale Ave, San Jose</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-3 animate-fade-in-up delay-600">
              {XUBIE_DATA.products.map((product) => (
                ...
                >
                  <span className="text-2xl block mb-1.5">{product.emoji}</span>
                  <p className="text-sm font-semibold text-[var(--foreground)] leading-snug group-hover:text-[var(--primary)] transition-colors line-clamp-2">
                    {product.name}
                  </p>
                  <p className="text-[11px] text-[var(--primary)] font-semibold mt-1.5">
                    from ${product.priceReg}
                  </p>
                </Link>
              ))}
            </div>
          ...
          <div className="lg:col-span-7 animate-fade-in-up delay-300">
            <div className="relative">
              ...
              {/* Third floating badge — freshness */}
              <div className="absolute top-1/2 -right-6 z-20 glass rounded-2xl px-3 py-2.5 shadow-lg animate-float hidden lg:flex items-center gap-2" style={{ animationDelay: "3s" }}>
                <span className="text-lg">🍮</span>
                <div>
                  <p className="text-[11px] font-bold text-[var(--foreground)]">Made Fresh</p>
                  <p className="text-[10px] text-[var(--muted-foreground)]">Every day</p>
                </div>
              </div>
